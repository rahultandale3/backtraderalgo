...
# -----------------------------------------------------------------------------------------------------------------------
    ...
    def notify_order(self, order):
        ...
        if order.status in [order.Completed]:
            if order.isbuy():
                if self.close_sl_s == 1 :
                    self.log('BUY EXECUTED CLOSE, %.2f' % self.data.close[-1])
                    self.close_sl_s = 0
                elif self.open_sl_s ==1 :
                    self.log('BUY EXECUTED OPEN, %.2f' % self.data.open[-1])
                    self.open_sl_s = 0

                else:
                    self.log('BUY EXECUTED, %.2f' % self.data.open[0])
            elif order.issell() :
                if self.close_sl_b ==1 :
                    self.log('SELL EXECUTED CLOSE, %.2f' % self.data.close[-1])
                    self.close_sl_b = 0
                elif self.close_sl_b ==1 :
                    self.log('SELL EXECUTED OPEN, %.2f' % self.data.open[-1])
                    self.open_sl_b = 0
                else:
